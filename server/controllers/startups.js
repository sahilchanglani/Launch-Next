...
export const getStartups = async (req, res) => { 
    ...
    try {
        ...
        const startups = await StartupModel.find().sort({_id: -1}).limit(LIMIT).skip(startIndex);
                
        res.status(200).json({ data: startups, currentPage: Number(page), numberOfPages: Math.ceil(total / LIMIT) });
    } catch (error) {
